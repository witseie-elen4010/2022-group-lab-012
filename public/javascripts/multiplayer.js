...
const display_square1 = document.querySelector('.opponent-basket')
...
const key_alphabets = ['Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P',
 ...
 document.getElementById('subBut').addEventListener('click', () => {
       let username = document.querySelector('#guess_word').value;
       localStorage.setItem('username', username);
     })
     var list = [1,2,3]
     var poll = Math.random()
     let leader = true // decided by results of poll NB: To do
     if(leader)
     {

        document.getElementById('guess_word').style.display = "flex"
     }
    
     else
     {
        document.getElementById('guess_word').style.display = "none"
     }

     
let guess_word = false
let wordle_word
function get_word() {
   /* if(guess_word)
    {
    ...
    {*/
        wordle_word = localStorage.getItem('username').toUpperCase()
    }   
...
// document.getElementById("demo").innerText =  wordle_word
...
function opponentBoard() {
    for (let i = 0; i < grid_row.length; i++) {
        ...
        display_square1.append(row_element)
    }
}
opponentBoard()
...
(()=>{
    ...
    get_word()
    const socket = io()
    const colour_change = handle_click()
    socket.emit('turn', click)
    socket.emit('feedback', colour_change) 
}
socket.on('turn', click)
socket.on('feedback', (colour_change) => start)
})()